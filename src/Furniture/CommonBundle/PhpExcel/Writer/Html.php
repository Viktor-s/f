...
class Html extends \PHPExcel_Writer_Abstract implements \PHPExcel_Writer_IWriter {
    /**
     * PHPExcel object
     *
     * @var \PHPExcel
            ...
            if ($drawing instanceof \PHPExcel_Worksheet_Drawing) {
                ...
                if ($drawing->getCoordinates() == $coordinates) {
                    switch ($drawing->getRenderingFunction()) {
                        ...
                        default:
                            throw new \RuntimeException('Undefined rendering function for drawing.');
                    }
                    ...
                    $base64 = base64_encode($blipData);
                    $imageData = 'data:' . $drawing->getMimeType() . ';base64,' . $base64;

                    $html .= '<div style="position: relative;">';
                    $html .= '<img style="position: relative; left: ' .
                        $drawing->getOffsetX() . 'px; top: ' . $drawing->getOffsetY() . 'px; width: ' .
                        $drawing->getWidth() . 'px; height: ' . $drawing->getHeight() . 'px;" src="' .
                        $imageData . '" border="0" />';
                    $html .= '</div>';
